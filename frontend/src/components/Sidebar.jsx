    ...
    >
      <div>
        ...
        <div className="flex items-center p-3 border-b border-gray-200">
          {isCollapsed ? (
            <div className="text-purple-600 font-bold text-xl mx-auto select-none">A</div>
          ) : (
        ...
        <nav className="p-2">
          <ul className="space-y-1">
            {[
              ...
              { to: '/distributed-tasks', icon: FaTasks, label: 'Distributed Tasks' },
            ].map((item) => (
              <li key={item.to}>
                <NavLink
                  to={item.to}
                  className={({ isActive }) =>
                    ...
                    ${isActive ? 'bg-purple-100 text-purple-600' : 'text-gray-600 hover:bg-purple-100 hover:text-purple-600'} 
                    font-medium`
                  }
                >
                  <item.icon
                    style={{ fontSize: isCollapsed ? '1.25rem' : '1.5rem', flexShrink: 0 }}
                  />
                  {!isCollapsed && <span className="text-sm select-none">{item.label}</span>}
                </NavLink>
              </li>
            ))}
          </ul>
      ...
      <div className="p-2 border-t border-gray-200">
        ...
        >
          <FaSignOutAlt style={{ fontSize: isCollapsed ? '1.25rem' : '1.5rem', flexShrink: 0 }} />
