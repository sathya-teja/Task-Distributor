...
// import Sidebar from '../components/Sidebar'; // Uncomment if you're using the Sidebar
...
function DistributedTasks() {
  ...
  useEffect(() => {
    const fetchDistributed = async () => {
      try {
        ...
        const endpoint = `${import.meta.env.VITE_BACKEND_URL}/api/lists/distributed-tasks`;
        ...
        const grouped = response.data.reduce((acc, task) => {
          ...
          const isDuplicate = acc[agentId].tasks.some(t =>
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50">
      ...
      <p className="mt-4 text-indigo-600 font-semibold text-lg">Loading tasks...</p>
    </div>
  );
  ...
  return (
    <div className="flex bg-gray-50">
      ...
      <main className="flex-1 p-4 sm:p-6 md:p-8 lg:p-10 bg-gray-50 max-w-7xl mx-auto w-full transition-all duration-300">
        <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-800 mb-4 sm:mb-6">Distributed Tasks by Agent</h2>
              ...
              <table className="w-full min-w-[320px] sm:min-w-[600px] border-collapse text-sm sm:text-base">
                <thead>
                  <tr className="bg-indigo-100">
                    <th className="py-2 px-3 sm:py-3 sm:px-4 text-left font-semibold text-indigo-900">First Name</th>
                    <th className="py-2 px-3 sm:py-3 sm:px-4 text-left font-semibold text-indigo-900">Phone</th>
                    <th className="py-2 px-3 sm:py-3 sm:px-4 text-left font-semibold text-indigo-900">Notes</th>
                  </tr>
                </thead>
                <tbody>
                  {agentGroup.tasks.map((task, index) => (
                    ...
                    >
                      <td className="py-2 px-3 sm:py-3 sm:px-4 text-gray-700 border-b border-gray-200">{task.firstName}</td>
                      <td className="py-2 px-3 sm:py-3 sm:px-4 text-gray-700 border-b border-gray-200">{task.phone}</td>
                      <td className="py-2 px-3 sm:py-3 sm:px-4 text-gray-700 border-b border-gray-200">{task.notes}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
